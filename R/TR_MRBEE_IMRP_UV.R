#' Estimate Non-Transferable Causal Effect with Univariable MRBEE and SuSiE
#'
...
TR_MRBEE_IMRP_UV=function(by,bx,byse,bxse,Rxy,theta.source,susie.iter=100,pip.thres=0.3,max.iter=30,max.eps=1e-4,pv.thres=0.05,var.est="variance",FDR=T,adjust.method="Sidak"){
by=by/byse
byseinv=1/byse
bx=bx*byseinv
bxse=bxse*byseinv
...
by.complement=c(by-bx*theta.complement)
fit.susie=susie(X=as.matrix(bx[indvalid]),y=by.complement[indvalid],L=1,estimate_prior_method="EM",s_init=fit.susie,standardize=F,max_iter=susie.iter,intercept=F)
h=sum(bx[indvalid]^2)-sum(bxse[indvalid]^2*Rxy[1,1])
if(fit.susie$pip>pip.thres){
g=sum(bx[indvalid]*by.complement[indvalid])-Rxy[1,2]*sum(bxse[indvalid]*byse[indvalid])
theta=g/h
}
