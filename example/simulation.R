...
A_eur=summary_generation(theta=theta_eur,m=m_eur,Rbb=SB_eur,Ruv=SE_eur,Rnn=Rnn,LD="identity",Nxy=rep(n_eur,11),non.zero.frac=rep(0.8,10),UHP.frac=UHP.frac,CHP.frac=0,UHP.var=UHP.var,Hxy=rep(h_eur,11),UHP.dis="uniform")
A_eas=summary_generation(theta=theta_eas,m=m_eas,Rbb=SB_eas,Ruv=SE_eas,Rnn=Rnn,LD="identity",Nxy=rep(n_eas,11),non.zero.frac=rep(0.8,10),UHP.frac=UHP.frac,CHP.frac=0,UHP.var=UHP.var,Hxy=rep(h_eas,11),UHP.dis="uniform")
bX_eur=A_eur$bX
by_eur=A_eur$by
bXse_eur=A_eur$bXse
byse_eur=A_eur$byse
...
t1=Sys.time()
fit_mrcml_susie=MRcML_IPOD_SuSiE(by=by_eas,bX=bX_eas,byse=byse_eas,bXse=bXse_eas,Rxy=Rxy_eas,L=fit_mrbee_susie$L.optimal+1,tau=fit_mrbee_susie$tau.optimal,theta.ini=fit_mrbee_susie$theta)
t2=Sys.time()
mrcml.susie.time=difftime(t2, t1, units = "secs")
...
t1=Sys.time()
fit_tran_mrbee=MRBEE_TL(by=by_eas,bX=bX_eas,byse=byse_eas,bXse=bXse_eas,Rxy=Rxy_eas,theta.source=fit_eur_mrbee$theta,theta.source.cov=fit_eur_mrbee$theta.cov,reliability.thres=0.8)
t2=Sys.time()
mrbee.tran.time=difftime(t2, t1, units = "secs")

t1=Sys.time()
fit_tran_mrcml=MRcML_TL(by=by_eas,bX=bX_eas,byse=byse_eas,bXse=bXse_eas,Rxy=Rxy_eas,theta.source=fit_eur_mrcml$theta,theta.source.cov=fit_eur_mrcml$theta.cov,L=fit_tran_mrbee$L.optimal+1,tau=fit_tran_mrbee$tau.optimal,theta.ini=fit_tran_mrbee$theta)
t2=Sys.time()
